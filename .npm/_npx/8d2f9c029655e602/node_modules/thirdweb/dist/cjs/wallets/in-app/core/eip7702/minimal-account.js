...
exports.create7702MinimalAccount = void 0;
...
const bundler_js_1 = require("../../../smart/lib/bundler.js");
...
const create7702MinimalAccount = (args) => {
    ...
    const _sendTxWithAuthorization = async (txs) => {
        ...
        const eoaContract = (0, contract_js_1.getContract)({
        ...
        if (sponsorGas) {
            ...
            const wrappedCalls = {
                calls: txs.map((tx) => ({
                    ...
                    target: (0, address_js_1.getAddress)(tx.to ?? ""),
                    value: tx.value ?? 0n,
                })),
                uid: (0, random_js_1.randomBytesHex)(),
            };
            ...
            const result = await (0, bundler_js_1.executeWithSignature)({
                authorization,
                eoaAddress: (0, address_js_1.getAddress)(adminAccount.address),
                options: {
        ...
        const executeTx = (0, execute_js_1.execute)({
            calls: txs.map((tx) => ({
                data: tx.data ?? "0x",
                target: tx.to ?? "",
                value: tx.value ?? 0n,
...
async function waitForTransactionHash(args) {
    ...
    while (Date.now() < endtime) {
        const result = await (0, bundler_js_1.getQueuedTransactionHash)({
