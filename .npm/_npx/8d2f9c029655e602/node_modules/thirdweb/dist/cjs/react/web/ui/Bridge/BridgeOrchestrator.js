...
function BridgeOrchestrator({ client, uiOptions, receiverAddress, onComplete, onError, onCancel, connectOptions, connectLocale, purchaseData, paymentLinkId, presetOptions, }) {
    ...
    return ((0, jsx_runtime_1.jsxs)(basic_js_1.Container, { flex: "column", fullHeight: true, children: [state.value === "error" && state.context.currentError && ((0, jsx_runtime_1.jsx)(ErrorBanner_js_1.ErrorBanner, { error: state.context.currentError, onCancel: onCancel, onRetry: handleRetry })), state.value === "init" && uiOptions.mode === "fund_wallet" && ((0, jsx_runtime_1.jsx)(FundWallet_js_1.FundWallet, { client: client, connectOptions: connectOptions, onContinue: handleRequirementsResolved, presetOptions: presetOptions, receiverAddress: receiverAddress, uiOptions: uiOptions })), state.value === "init" && uiOptions.mode === "direct_payment" && ((0, jsx_runtime_1.jsx)(DirectPayment_js_1.DirectPayment, { client: client, connectOptions: connectOptions, onContinue: handleRequirementsResolved, uiOptions: uiOptions })), state.value === "init" && uiOptions.mode === "transaction" && ((0, jsx_runtime_1.jsx)(TransactionPayment_js_1.TransactionPayment, { client: client, connectOptions: connectOptions, onContinue: handleRequirementsResolved, uiOptions: uiOptions })), state.value === "methodSelection" &&
                state.context.destinationToken &&
                state.context.destinationAmount &&
                state.context.receiverAddress && ((0, jsx_runtime_1.jsx)(PaymentSelection_js_1.PaymentSelection, { client: client, connectLocale: connectLocale || en_js_1.default, connectOptions: connectOptions, destinationAmount: state.context.destinationAmount, destinationToken: state.context.destinationToken, includeDestinationToken: uiOptions.mode !== "fund_wallet", onBack: () => {
                    send({ type: "BACK" });
                }, onError: handleError, onPaymentMethodSelected: handlePaymentMethodSelected, receiverAddress: state.context.receiverAddress })), state.value === "quote" &&
                ...
                state.context.destinationAmount && ((0, jsx_runtime_1.jsx)(QuoteLoader_js_1.QuoteLoader, { amount: state.context.destinationAmount, client: client, destinationToken: state.context.destinationToken, onBack: () => {
                    send({ type: "BACK" });
                }, onError: handleError, onQuoteReceived: handleQuoteReceived, paymentLinkId: paymentLinkId, paymentMethod: state.context.selectedPaymentMethod, purchaseData: purchaseData, receiver: state.context.receiverAddress })), state.value === "preview" &&
                state.context.selectedPaymentMethod &&
                ...
                state.context.quote &&
                state.context.completedStatuses && ((0, jsx_runtime_1.jsx)(SuccessScreen_js_1.SuccessScreen, { completedStatuses: state.context.completedStatuses, onDone: handleBuyComplete, preparedQuote: state.context.quote, uiOptions: uiOptions, windowAdapter: WindowAdapter_js_1.webWindowAdapter })), state.value === "post-buy-transaction" &&
