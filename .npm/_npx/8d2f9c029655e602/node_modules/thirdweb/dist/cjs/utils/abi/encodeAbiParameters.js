...
exports.prepareParam = prepareParam;
exports.encodeAddress = encodeAddress;
const ox__Hex = require("ox/Hex");
const address_js_1 = require("../address.js");
const byte_size_js_1 = require("../encoding/helpers/byte-size.js");
...
function encodeParams(preparedParams) {
    ...
    for (let i = 0; i < preparedParams.length; i++) {
        ...
        const { dynamic, encoded } = preparedParams[i];
        if (dynamic) {
            staticParams.push((0, hex_js_1.numberToHex)(staticSize + dynamicSize, { size: 32 }));
...
function encodeAddress(value) {
    ...
    return { dynamic: false, encoded: (0, hex_js_1.padHex)(value.toLowerCase()) };
}
function encodeArray(value, { length, param, }) {
    ...
    if (dynamic || dynamicChild) {
        const data = encodeParams(preparedParams);
        if (dynamic) {
            const length_ = (0, hex_js_1.numberToHex)(preparedParams.length, { size: 32 });
            return {
                dynamic: true,
...
function encodeBytes(value, { param }) {
    ...
    if (!paramSize) {
        ...
        if (bytesSize % 32 !== 0) {
            value_ = (0, hex_js_1.padHex)(value_, {
                dir: "right",
        ...
        return {
            dynamic: true,
            encoded: ox__Hex.concat(...[(0, hex_js_1.padHex)((0, hex_js_1.numberToHex)(bytesSize, { size: 32 })), value_]),
        };
    }
    if (bytesSize !== Number.parseInt(paramSize)) {
        throw new Error(`Invalid bytes${paramSize} size: ${bytesSize}`);
    }
    return { dynamic: false, encoded: (0, hex_js_1.padHex)(value, { dir: "right" }) };
}
function encodeBool(value) {
    return { dynamic: false, encoded: (0, hex_js_1.padHex)((0, hex_js_1.boolToHex)(value)) };
}
function encodeNumber(value, { signed }) {
    return {
        dynamic: false,
        encoded: (0, hex_js_1.numberToHex)(value, {
            signed,
            size: 32,
        }),
    };
...
function encodeString(value) {
    const hexValue = (0, hex_js_1.stringToHex)(value);
    const partsLength = Math.ceil((0, byte_size_js_1.byteSize)(hexValue) / 32);
    const parts = [];
    for (let i = 0; i < partsLength; i++) {
        parts.push((0, hex_js_1.padHex)(ox__Hex.slice(hexValue, i * 32, (i + 1) * 32), {
            dir: "right",
        }));
    }
    return {
        dynamic: true,
        encoded: ox__Hex.concat(...[(0, hex_js_1.padHex)((0, hex_js_1.numberToHex)((0, byte_size_js_1.byteSize)(hexValue), { size: 32 })), ...parts]),
    };
}
