...
export function PayTokenIcon(props) {
    ...
    const tokenIcon = !isNativeToken(token)
        ? supportedDestinationsQuery.data
            ...
            ?.tokens.find((t) => getAddress(t.address) === getAddress(token.address))?.icon
        : undefined;
    return (_jsx(TokenIcon, { chain: props.chain, client: props.client, size: props.size, token: isNativeToken(token)
            ? { nativeToken: true }
            : {
