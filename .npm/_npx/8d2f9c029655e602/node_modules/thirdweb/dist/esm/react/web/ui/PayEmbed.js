...
export function PayEmbed(props) {
    ...
    const siweAuth = useSiweAuth(activeWallet, activeAccount, props.connectOptions?.auth);
    ...
    if (props.payOptions?.mode === "fund_wallet" &&
        ...
        return (_jsx(BuyWidget, { amount: props.payOptions.prefillBuy.amount || "0.01", chain: props.payOptions.prefillBuy.chain, client: props.client, theme: theme, title: metadata?.name || "Buy", tokenAddress: props.payOptions.prefillBuy.token?.address }));
    }
    ...
    if (props.payOptions?.mode === "transaction") {
        return (_jsx(TransactionWidget, { client: props.client, description: metadata?.description, image: metadata?.image, theme: theme, title: metadata?.name, transaction: props.payOptions.transaction }));
    }
    if (!localeQuery.data) {
        ...
        content = (_jsxs(_Fragment, { children: [_jsx(AutoConnect, { client: props.client, siweAuth: siweAuth }), screen === "buy" && (_jsx(BuyScreen, { client: props.client, connectLocale: localeQuery.data, connectOptions: props.connectOptions, hiddenWallets: props.hiddenWallets, isEmbed: true, onBack: undefined, onDone: () => {
                        if (props.payOptions?.mode === "transaction") {
