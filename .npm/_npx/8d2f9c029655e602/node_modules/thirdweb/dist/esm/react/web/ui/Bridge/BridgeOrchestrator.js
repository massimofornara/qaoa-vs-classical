...
export function BridgeOrchestrator({ client, uiOptions, receiverAddress, onComplete, onError, onCancel, connectOptions, connectLocale, purchaseData, paymentLinkId, presetOptions, }) {
    ...
    return (_jsxs(Container, { flex: "column", fullHeight: true, children: [state.value === "error" && state.context.currentError && (_jsx(ErrorBanner, { error: state.context.currentError, onCancel: onCancel, onRetry: handleRetry })), state.value === "init" && uiOptions.mode === "fund_wallet" && (_jsx(FundWallet, { client: client, connectOptions: connectOptions, onContinue: handleRequirementsResolved, presetOptions: presetOptions, receiverAddress: receiverAddress, uiOptions: uiOptions })), state.value === "init" && uiOptions.mode === "direct_payment" && (_jsx(DirectPayment, { client: client, connectOptions: connectOptions, onContinue: handleRequirementsResolved, uiOptions: uiOptions })), state.value === "init" && uiOptions.mode === "transaction" && (_jsx(TransactionPayment, { client: client, connectOptions: connectOptions, onContinue: handleRequirementsResolved, uiOptions: uiOptions })), state.value === "methodSelection" &&
                state.context.destinationToken &&
                state.context.destinationAmount &&
                state.context.receiverAddress && (_jsx(PaymentSelection, { client: client, connectLocale: connectLocale || en, connectOptions: connectOptions, destinationAmount: state.context.destinationAmount, destinationToken: state.context.destinationToken, includeDestinationToken: uiOptions.mode !== "fund_wallet", onBack: () => {
                    send({ type: "BACK" });
                ...
                state.context.quote &&
                state.context.completedStatuses && (_jsx(SuccessScreen, { completedStatuses: state.context.completedStatuses, onDone: handleBuyComplete, preparedQuote: state.context.quote, uiOptions: uiOptions, windowAdapter: webWindowAdapter })), state.value === "post-buy-transaction" &&
