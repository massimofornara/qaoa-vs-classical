...
export function useSendTransaction(config = {}) {
    ...
    const showPayModal = (data) => {
        if (payModal === false)
            return;
        setRootEl(_jsx(TransactionModal, { client: data.tx.client, localeId: payModal?.locale || "en_US", modalMode: data.mode, onClose: () => {
                setRootEl(null);
                ...
                transaction: data.tx,
            }, supportedTokens: payModal?.supportedTokens, theme: payModal?.theme || "dark", title: payModal?.metadata?.name || "Transaction", tx: data.tx, txId: randomBytesHex() }));
    };
