...
export function ConnectButton(props) {
    ...
    return (_jsxs(View, { children: [isConnectedAndAuth ? (_jsx(ConnectedButton, { account: account, onClose: closeModal, openModal: () => openModal(), wallet: wallet, ...props })) : (_jsx(ThemedButton, { onPress: () => openModal(), theme: theme, children: status === "connecting" ||
                    siweAuth.isLoggingIn ||
                    siweAuth.isLoading ||
                    siweAuth.isLoggingOut ? (_jsx(ThemedSpinner, { color: theme.colors.primaryButtonText })) : (_jsx(ThemedText, { style: { color: theme.colors.primaryButtonText }, theme: theme, type: "defaultSemiBold", children: isConnectedAndNotAuth
                        ? props.signInButton?.label || "Sign In"
                        : props.connectButton?.label || "Connect Wallet" })) })), _jsx(Modal, { animationType: "none", onRequestClose: closeModal, transparent: true, visible: visible, children: _jsxs(KeyboardAvoidingView, { behavior: "padding", style: { flex: 1 }, children: [_jsx(Animated.View, { style: [styles.modalOverlay, { opacity: fadeAnim.current }] }), _jsxs(Animated.View, { style: [
                                { flex: 1, transform: [{ translateY: slideAnim.current }] },
                            ], children: [_jsx(Pressable, { onPress: closeModal, style: styles.dismissArea }), _jsx(View, { style: styles.bottomSheetContainer, children: isConnectedAndAuth ? (_jsx(ConnectedModal, { ...props, account: account, containerType: "modal", onClose: closeModal, theme: theme, wallet: wallet })) : (_jsx(ConnectModal, { ...props, containerType: "modal", onClose: closeModal, siweAuth: siweAuth, theme: theme })) })] })] }) })] }));
}
