...
import { useSetIsWalletModalOpen, WalletUIStatesProvider, } from "../../providers/wallet-ui-states-provider.js";
...
export function ConnectButton(props) {
    ...
    const autoConnectComp = props.autoConnect !== false && (_jsx(AutoConnect, { accountAbstraction: props.accountAbstraction, appMetadata: props.appMetadata, chain: preferredChain, client: props.client, onConnect: props.onConnect, siweAuth: siweAuth, timeout: typeof props.autoConnect === "boolean"
            ? undefined
            : props.autoConnect?.timeout, wallets: wallets }));
    if (!localeQuery.data) {
        const combinedClassName = `${props.connectButton?.className || ""} ${TW_CONNECT_WALLET}`;
        return (_jsxs(AnimatedButton, { className: combinedClassName, disabled: true, style: {
                height: "50px",
                minWidth: "165px",
                ...props.connectButton?.style,
            }, type: "button", variant: "primary", children: [autoConnectComp, _jsx(Spinner, { color: "primaryButtonText", size: "sm" })] }));
    }
    return (_jsxs(WalletUIStatesProvider, { isOpen: false, theme: props.theme, children: [_jsx(ConnectButtonInner, { ...props, connectLocale: localeQuery.data, siweAuth: siweAuth }), _jsx(ConnectModal, { accountAbstraction: props.accountAbstraction, auth: props.auth, chain: preferredChain, chains: props.chains, client: props.client, connectLocale: localeQuery.data, hiddenWallets: hiddenWallets, localeId: props.locale || "en_US", meta: {
...
function ConnectButtonInner(props) {
    ...
    if (!activeAccount) {
        ...
        const combinedClassName = `${props.connectButton?.className || ""} ${TW_CONNECT_WALLET}`;
        return (_jsx(AnimatedButton, { "aria-label": connectionStatus === "connecting"
                ? locale.connecting
                ...
                minWidth: "165px",
                ...props.connectButton?.style,
            }, type: "button", variant: "primary", children: isLoading ? (_jsx(Spinner, { color: "primaryButtonText", size: "sm" })) : (connectButtonLabel) }));
    }
    if (siweAuth.requiresAuth) {
        ...
        if (siweAuth.isPending || siweAuth.isLoggingIn || siweAuth.isLoggingOut) {
            const combinedClassName = `${props.connectButton?.className || ""} ${TW_CONNECT_WALLET}`;
            return (_jsx(AnimatedButton, { className: combinedClassName, disabled: true, style: {
                    height: "50px",
                    minWidth: "165px",
                    ...props.connectButton?.style,
                }, type: "button", variant: "primary", children: _jsx(Spinner, { color: "primaryButtonText", size: "sm" }) }));
        }
        // sign in button + modal if *not* loading and *not* logged in
        if (!siweAuth.isLoggedIn) {
            return (_jsxs(_Fragment, { children: [_jsx(Button, { className: props.signInButton?.className, onClick: () => {
    ...
    return (_jsx(AccountProvider, { address: activeAccount.address, client: props.client, children: _jsx(ConnectedWalletDetails, { chain: props.chain, chains: props?.chains || [], client: props.client, connectLocale: locale, connectOptions: {
