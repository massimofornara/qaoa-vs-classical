...
export const ConnectWalletSocialOptions = (props) => {
    ...
    return (_jsxs(Container, { flex: "column", gap: "md", style: {
            position: "relative",
        }, children: [optionalImageMetadata && (_jsxs(_Fragment, { children: [_jsx(Img, { alt: optionalImageMetadata.alt, client: props.client, height: `${optionalImageMetadata.height}`, src: optionalImageMetadata.src, style: { margin: "auto" }, width: `${optionalImageMetadata.width}` }), _jsx(Spacer, { y: "xxs" })] })), hasSocialLogins && (_jsx(Container, { flex: "column", gap: socialLogins.length > 4 ? "xs" : "sm", children: socialLoginColumns.map((column) => (_jsx(SocialButtonRow, { children: column.map((loginMethod) => {
                        const imgIconSize = (() => {
                            if (!showOnlyIcons) {
                                return iconSize.md;
                            }
                        ...
                        return (_jsxs(SocialButton, { "aria-label": `Login with ${loginMethod}`, "data-variant": showOnlyIcons ? "icon" : "full", disabled: props.disabled, onClick: () => {
                                handleSocialLogin(loginMethod);
                            }, style: {
                                flexGrow: socialLogins.length < 7 ? 1 : 0,
                            }, variant: "outline", children: [_jsx(Img, { client: props.client, height: imgIconSize, src: socialIcons[loginMethod], width: imgIconSize }), !showOnlyIcons &&
                ...
                (inputMode === "email" ? (_jsx(InputSelectionUI, { disabled: props.disabled, emptyErrorMessage: emptyErrorMessage, errorMessage: (input) => {
                        ...
                        props.select();
                    }, placeholder: placeholder, submitButtonText: locale.submitEmail, type: type })) : (_jsx(WalletTypeRowButton, { client: props.client, disabled: props.disabled, icon: EmailIcon, onClick: () => {
                        setManualInputMode("email");
                    }, title: locale.emailPlaceholder }))), isPhoneEnabled &&
                (inputMode === "phone" ? (_jsx(InputSelectionUI, { allowedSmsCountryCodes: wallet.getConfig()?.auth?.allowedSmsCountryCodes, defaultSmsCountryCode: wallet.getConfig()?.auth?.defaultSmsCountryCode, disabled: props.disabled, emptyErrorMessage: emptyErrorMessage, errorMessage: (_input) => {
                        ...
                        props.select();
                    }, placeholder: placeholder, submitButtonText: locale.submitEmail, type: type })) : (_jsx(WalletTypeRowButton, { client: props.client, disabled: props.disabled, icon: PhoneIcon, onClick: () => {
                        setManualInputMode("phone");
                    }, title: locale.phonePlaceholder }))), passKeyEnabled && (_jsx(WalletTypeRowButton, { client: props.client, disabled: props.disabled, icon: FingerPrintIcon, onClick: () => {
                    handlePassKeyLogin();
                }, title: locale.passkey })), siweEnabled && !props.isLinking && (_jsx(WalletTypeRowButton, { client: props.client, icon: OutlineWalletIcon, onClick: () => {
                    handleWalletLogin();
                }, title: locale.signInWithWallet })), guestEnabled && (_jsx(WalletTypeRowButton, { client: props.client, disabled: props.disabled, icon: GuestIcon, onClick: () => {
                    handleGuestLogin();
                }, title: locale.loginAsGuest })), props.isLinking && (_jsx(WalletTypeRowButton, { client: props.client, icon: OutlineWalletIcon, onClick: () => {
                    handleWalletLogin();
                }, title: locale.linkWallet }))] }));
};
const SocialButtonRow = (props) => (_jsx(Container, { center: "x", flex: "row", gap: props.children.length > 4 ? "xs" : "sm", style: {
        display: "flex",
