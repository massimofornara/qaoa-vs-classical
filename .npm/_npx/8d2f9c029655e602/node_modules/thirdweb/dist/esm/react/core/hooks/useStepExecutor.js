...
import { useBridgePrepare, } from "./useBridgePrepare.js";
/**
...
export function useStepExecutor(options) {
    ...
    const flatTxs = useMemo(() => (preparedQuote?.steps ? flattenRouteSteps(preparedQuote.steps) : []), [preparedQuote?.steps]);
    // State management
    ...
    const [onrampStatus, setOnrampStatus] = useState(preparedQuote?.type === "onramp" ? "pending" : undefined);
    useQuery({
        queryFn: async () => {
            if (!isLoading) {
    ...
    const executeSingleTx = useCallback(async (tx, account, completedStatusResults, abortSignal) => {
        ...
        const { prepareTransaction } = await import("../../../transaction/prepare-transaction.js");
        const { sendTransaction } = await import("../../../transaction/actions/send-transaction.js");
    ...
    const executeBatch = useCallback(async (txs, account, completedStatusResults, abortSignal) => {
        ...
        const { prepareTransaction } = await import("../../../transaction/prepare-transaction.js");
        const { sendBatchTransaction } = await import("../../../transaction/actions/send-batch-transaction.js");
